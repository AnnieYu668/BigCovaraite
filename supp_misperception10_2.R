...
ParCorsq1 <- Vectorize(ParCorsq1,"alphaz") 

DevFun1 <- function(alphaz, gam,beta,w){
  w = 1
  alphax = 1  #alphaz*w
...
Dev0 <- grad(DevFun1, alphaz[alphaz >0], gam = gam, beta=beta, w=1)
plot(alphaz[alphaz >0], Dev0)


Y00 <- ParCorsq1(gam,beta, alphaz[alphaz >0], w)
plot(alphaz[alphaz >0], Y00)



#--- Y.X'|Z'
ParCorsq <- function(gam,beta, alphaz, w){
  alphax = alphaz*w
...
ParCorsq <- Vectorize(ParCorsq,"alphaz") 
...
ParCorsqF2 <- function(gam,beta, alphaz, w){
  alphax = alphaz*w
...
ParCorsqF2 <- Vectorize(ParCorsqF2,"alphaz") 
...
DevFun2 <- function(alphaz, gam,beta,w){
  alphax = alphaz*w
...
plot(alphaz, Dev0, type = "l", xlab=expression(alpha[z]), ylab="", lwd=6, col="red")
#abline(h=0.5,lwd=5, lty=2,col="lightgrey")
abline(v=1.0,lwd=5, lty=2,col="lightgrey")

Dev02 <- grad(DevFun2, alphaz[alphaz >0], gam = gam, beta=beta, w=2)
plot(alphaz[alphaz >0], Dev02)
abline(h = 0,lwd=3)
...
min(-1/(gam*alphaz*alphaz))
range(1/(gam*alphaz*alphaz))
range(( (1 - a0*gam*alphaz)/alphaz))
alphaz[which(1/(gam*alphaz*alphaz) == range(1/(gam*alphaz*alphaz))[1] )]


w <- seq(min(-1/(gam*alphaz*alphaz)), max(1/(gam*alphaz*alphaz)), length.out = 5)
w <- seq(-2, 2, length.out = 10)

Res <- matrix(0,nrow=length(alphaz), ncol=length(w))

for(i in 1:length(w)){
  
  Res[,i] <- ParCorsq(gam,beta, alphaz, w[i]) 
}

matplot(alphaz, Res, type ="l",col=1:10,lwd=3)

plot(alphaz, ParCorsq(gam,beta, alphaz,1.1))

range(ParCorsq(gam,beta, alphaz,1.1) - ParCorsqF2(gam,beta, alphaz,1))

plot(alphaz, ParCorsq0(gam,beta, alphaz,1, 0),type = "l", xlab=expression(alpha[z]), ylab="", lwd=6, col="red")
plot(alphaz, ParCorsq0(1,1, alphaz,-.15, -.85),type = "l", xlab=expression(alpha[z]), ylab="", lwd=6, col="red")

plot(alphaz,ParCorsq(gam,beta, alphaz, -1.00), type = "l", xlab=expression(alpha[z]), ylab="", lwd=6, col="red")
abline(h=0.5,lwd=5, lty=2,col="lightgrey")
abline(v=1.0,lwd=5, lty=2,col="lightgrey")
points(alphaz, ParCorsqF2(gam,beta, alphaz,-1.00),col="red")


#-- find combinason of gam, beta, and 
#-----------------------------------------------
DevFun2 <- function(alphaz, gam,beta,w){
  alphax = alphaz*w
...
Finfunc <- function(gam, beta){
  alphaz = seq(0.001, .999,length=200)
  #w = 1/(gam*max(alphaz)*max(alphaz))
  
  w = min((1 - a0*gam*alphaz)/alphaz)
  Dev02 <- grad(DevFun2, alphaz, gam = gam, beta=beta, w=w)
  mean(Dev02 < 0)
}
...
gam = beta =seq(0.001, .999,length.out = 50)
resFin <- outer(gam,beta, "Finfunc")